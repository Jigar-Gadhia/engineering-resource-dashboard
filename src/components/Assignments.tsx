...
const Assignments: React.FC = () => {
  ...
  const getSkillMatch = (engineerId: string, projectId: string) => {
    ...
    const matched = eng.skills.filter((skill) =>
      proj.required_skills.includes(skill)
    );
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between">
        <div className="mb-5">
          ...
          <p className="text-muted-foreground mt-1">
            Manage engineer assignments to projects
          </p>
        </div>
        {user?.role === "manager" && (
          <Dialog open={showDialog} onOpenChange={setShowDialog}>
            ...
            <DialogContent>
              <DialogHeader>
                ...
                <DialogDescription>
                  Fill the form below to create assignment.
                </DialogDescription>
              </DialogHeader>
              <form className="space-y-4" onSubmit={handleCreate}>
                ...
                <Select
                  ...
                  onValueChange={(v) =>
                    setNewAssignment({ ...newAssignment, engineer_id: v })
                  }
                >
                  <SelectTrigger>
                ...
                {newAssignment.project_id && newAssignment.engineer_id && (
                  <p className="text-sm">
                    ...
                    <span
                      className={`${
                        getSkillMatch(
                          newAssignment.engineer_id,
                          newAssignment.project_id
                        ) >= 80
                          ? "text-green-500"
                          : getSkillMatch(
                              newAssignment.engineer_id,
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-x-8 gap-y-10">
        {assignments.map((a) => {
          ...
          return (
            <Card
              key={a.id}
              className="dark:bg-muted/50 h-full flex flex-col justify-between p-4 shadow-sm border border-border"
            >
              ...
              <CardContent className="space-y-2 text-sm p-0">
                ...
                <div className="flex items-center gap-2 text-muted-foreground">
                  ...
                  {a.allocated_hours} hrs
                </div>
                <div className="flex items-center gap-2 text-muted-foreground">
                  <Calendar className="h-4 w-4 text-purple-400" /> Start:{" "}
                  {moment(a.start_date).format("DD/MM/YYYY")}
                </div>
                <div className="flex items-center gap-2 text-muted-foreground">
                  <Calendar className="h-4 w-4 text-red-400" /> End:{" "}
                  {moment(a.end_date).format("DD/MM/YYYY")}
                </div>
